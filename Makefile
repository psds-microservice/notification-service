.PHONY: help build run run-dev clean tidy vet fmt health-check proto proto-build proto-generate proto-generate-local proto-generate-docker proto-openapi install-deps update docker-build docker-compose-up docker-compose-down

APP_NAME = notification-service
CMD_PATH = ./cmd/notification-service
BIN_DIR = bin
PORT = 8092
PROTOC_IMAGE = local/protoc-go:latest
PROTO_ROOT = pkg/notification_service
PROTO_FILE = notification.proto
GEN_DIR = pkg/gen/notification_service
GO_MODULE = github.com/psds-microservice/notification-service
OPENAPI_OUT = api
.DEFAULT_GOAL := help
